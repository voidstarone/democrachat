...
#[tokio::test]
async fn a_user_publishes_keys_and_recovers_them_with_their_password() {
    ...
    s.keys().publish_keys("alice", &public_hex, WrappedKey::from(wrapped.clone())).await.unwrap();

    // Server hands the entry back verbatim; the public key round-trips.
    let mine = s.keys().my_keys("alice").await.unwrap();
...
#[tokio::test]
async fn the_wrapped_secret_is_useless_without_the_password() {
    let s = with_users(&["alice"]).await;
    let secret = IdentitySecret::generate();
    let wrapped = wrap_secret("the-real-password-000", &secret).unwrap();
    s.keys().publish_keys("alice", &secret.public().to_hex(), WrappedKey::from(wrapped)).await.unwrap();
...
#[tokio::test]
async fn a_sender_seals_to_a_fetched_public_key_and_only_the_owner_opens_it() {
    ...
    s.keys().publish_keys("bob", &bob_secret.public().to_hex(), WrappedKey::from(bob_wrapped)).await.unwrap();

    // Alice fetches bob's PUBLIC key (no secret is ever exposed) and seals to it.
    let bob_pub_hex = s.keys().public_key_of("bob").await.unwrap();
    ...
    let bob_mine = s.keys().my_keys("bob").await.unwrap();
...
#[tokio::test]
async fn a_malformed_public_key_is_rejected() {
    let s = with_users(&["alice"]).await;
    let secret = IdentitySecret::generate();
    let wrapped = wrap_secret("some-long-password-01", &secret).unwrap();
    let err = s.keys().publish_keys("alice", "not-a-real-key", WrappedKey::from(wrapped)).await.unwrap_err();
...
#[tokio::test]
async fn fetching_before_publishing_reports_not_published() {
    let s = with_users(&["alice"]).await;
    assert!(matches!(s.keys().my_keys("alice").await, Err(KeyError::NotPublished(_))));
    assert!(matches!(s.keys().public_key_of("alice").await, Err(KeyError::NotPublished(_))));
    assert!(matches!(s.keys().public_key_of("ghost").await, Err(KeyError::NoSuchUser(_))));
}
