...
fn pass(f: &Fixture, kind: ProposalKind) {
    let p = f.services.open_proposal("ada", "town-square", kind).unwrap();
    for v in ["ada", "bob", "cid"] {
        f.services.cast_vote(v, p.id.0, true).unwrap();
    }
...
fn a_role_is_created_only_by_ballot() {
    ...
    let roles = f.services.list_roles("town-square");
...
fn a_member_is_assigned_to_a_role_by_ballot_and_shows_as_a_holder() {
    ...
    assert_eq!(f.services.role_holders("town-square", "mapmakers"), vec!["bob".to_string()]);
}
...
fn a_role_mention_resolves_to_its_holders() {
    ...
    let resolved = f.services.resolve_mentions("town-square", "ping @mapmakers please");
...
fn the_citizens_standing_role_addresses_every_citizen() {
    ...
    let resolved = f.services.resolve_mentions("town-square", "@citizens assemble");
...
fn a_user_mention_resolves_to_that_member() {
    let f = fixture(1_000 * DAY);
    setup(&f);
    let resolved = f.services.resolve_mentions("town-square", "hi @bob");
...
fn an_unknown_or_non_member_mention_is_marked_unknown() {
    ...
    let resolved = f.services.resolve_mentions("town-square", "@nobody and @stranger");
...
fn deleting_a_role_removes_its_assignments() {
    ...
    let bob = f.store.find_by_handle("bob").unwrap();
    pass(&f, ProposalKind::AssignRole { user: bob.id, role: role.id });
    assert_eq!(f.services.role_holders("town-square", "temp").len(), 1);
...
fn mentioned_handles_dedupes_across_user_and_role() {
    ...
    let handles = f.services.mentioned_handles("town-square", "@bob and @crew");
