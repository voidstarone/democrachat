...
fn contribution(f: &Fixture, handle: &str) -> i64 {
    ...
    let server = f.store.find_by_slug("gamers").unwrap().unwrap();
    f.store.get(user.id, server.id).unwrap().unwrap().contribution
}
...
fn backfill_gives_channelless_servers_their_floor_channels() {
    ...
    let sid = store.next_server_id().unwrap();
    store.insert_server(domain::Server::new(sid, "old-town", "Old Town", domain::UserId(1), Timestamp(1_000 * DAY))).unwrap();
    let _ = &clock;
    assert!(services.chat().list_channels("old-town").unwrap().is_empty(), "starts channelless");

    services.backfill_default_channels();
    let names: Vec<String> =
        services.chat().list_channels("old-town").unwrap().into_iter().map(|c| c.name).collect();
    ...
    services.backfill_default_channels();
    assert_eq!(services.chat().list_channels("old-town").unwrap().len(), 2);
}
...
fn seed_server(f: &Fixture) {
    ...
    let user = f.store.find_by_handle("alice").unwrap().unwrap();
    let server = f.store.find_by_slug("gamers").unwrap().unwrap();
    assert_eq!(f.store.get(user.id, server.id).unwrap().unwrap().tier, Tier::Citizen);
}
...
fn a_citizen_reaction_endorses_the_author_but_a_members_does_not() {
    ...
    let msg = f.services.chat().post_message("bob", "gamers", "general", "hello world").unwrap();

    // carol is only a Member — her reaction must NOT endorse.
    f.services.chat().react("carol", msg.id.0, "👍").unwrap();
    ...
    f.services.chat().react("alice", msg.id.0, "🎉").unwrap();
    ...
    f.services.chat().unreact("alice", msg.id.0, "🎉").unwrap();
    assert_eq!(contribution(&f, "bob"), 1);
...
fn self_reactions_never_endorse() {
    let f = fixture(1_000 * DAY);
    seed_server(&f);
    let msg = f.services.chat().post_message("alice", "gamers", "general", "look at me").unwrap();
    f.services.chat().react("alice", msg.id.0, "👍").unwrap();
...
fn only_the_author_may_edit() {
    ...
    f.services.register_account("bob").unwrap();
    f.services.join_server("bob", "gamers").unwrap();
    let msg = f.services.chat().post_message("bob", "gamers", "general", "typo").unwrap();
    assert!(f.services.chat().edit_message("alice", msg.id.0, "hijack").is_err());
    assert!(f.services.chat().edit_message("bob", msg.id.0, "fixed").is_ok());
}
...
fn non_members_cannot_post_and_only_the_founder_provisions_in_seed() {
    ...
    assert!(f.services.chat().create_channel("stranger", "gamers", "random", "").is_err());
    // A non-member cannot post.
    assert!(f.services.chat().post_message("stranger", "gamers", "general", "hi").is_err());
}
...
fn replies_thread_under_their_parent() {
    let f = fixture(1_000 * DAY);
    seed_server(&f);
    let root = f.services.chat().post_message("alice", "gamers", "general", "topic").unwrap();
    let reply = f.services.chat().reply_message("alice", root.id.0, "reply").unwrap();
    assert_eq!(reply.parent, Some(root.id));

    let msgs = f.services.chat().channel_messages("gamers", "general").unwrap();
...
fn a_deleted_message_is_tombstoned_and_keeps_its_replies() {
    let f = fixture(1_000 * DAY);
    seed_server(&f);
    let root = f.services.chat().post_message("alice", "gamers", "general", "parent").unwrap();
    let reply = f.services.chat().reply_message("alice", root.id.0, "child").unwrap();
    f.services.chat().delete_message("alice", root.id.0).unwrap();
...
fn history_sharing_hides_a_members_past_messages_from_later_joiners() {
    ...
    let early = s.chat().post_message("bob", "gamers", "general", "early hello").unwrap();
    ...
    let late = s.chat().post_message("bob", "gamers", "general", "later hello").unwrap();

    let ids = |viewer: &str| -> Vec<u64> {
        s.chat().channel_messages_for(viewer, "gamers", "general").unwrap().iter().map(|m| m.id.0).collect()
    };
    ...
    s.chat().set_history_sharing("bob", "gamers", false).unwrap();
...
fn deleting_a_message_deletes_its_media() {
    ...
    f.services.chat().delete_message("alice", msg.id.0).unwrap();
