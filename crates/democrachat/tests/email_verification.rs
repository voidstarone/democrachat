...
#[test]
fn hard_mode_register_is_unverified_and_issues_a_token() {
    ...
    // Even the login credential path returns the account (the verified gate is the
    // web layer's job, not authenticate's).
    assert!(s.authenticate("alice", GOOD_PW).is_some());
}

#[test]
fn the_stored_email_is_ciphertext_not_plaintext() {
    let (s, _clock) = hard_services();
...
#[test]
fn verifying_marks_the_account_and_is_single_use() {
    let (s, _clock) = hard_services();
    let token = s
        .register_with_password("alice", "alice@example.com", GOOD_PW)
        .unwrap()
    ...
    let verified = s.verify_email(&token).expect("valid token verifies");
    assert!(verified.is_email_verified());
    // The account is now verified in the store.
    assert!(s.find_user("alice").unwrap().is_email_verified());
    // ...and the token cannot be replayed.
    assert!(s.verify_email(&token).is_none(), "token is single-use");
}
...
fn an_unknown_token_does_not_verify() {
    let (s, _clock) = hard_services();
    s.register_with_password("alice", "alice@example.com", GOOD_PW).unwrap();
    assert!(s.verify_email("not-a-real-token").is_none());
    assert!(!s.find_user("alice").unwrap().is_email_verified());
}
...
fn an_expired_token_does_not_verify() {
    ...
    assert!(s.verify_email(&token).is_none(), "expired token is rejected");
    assert!(!s.find_user("alice").unwrap().is_email_verified());
}

#[test]
fn a_duplicate_email_is_rejected_case_insensitively() {
    let (s, _clock) = hard_services();
    s.register_with_password("alice", "alice@example.com", GOOD_PW).unwrap();
    match s.register_with_password("bob", "ALICE@Example.com", GOOD_PW) {
        Err(RegisterError::EmailTaken) => {}
        other => panic!("expected EmailTaken, got {other:?}"),
    }
    assert!(s.find_user("bob").is_none(), "no account created on clash");
}

#[test]
fn a_malformed_email_is_rejected() {
    let (s, _clock) = hard_services();
    match s.register_with_password("alice", "not-an-email", GOOD_PW) {
...
#[test]
fn resend_returns_the_address_and_a_working_token_until_verified() {
    let (s, _clock) = hard_services();
    s.register_with_password("alice", "alice@example.com", GOOD_PW).unwrap();

    let target = s.issue_resend("alice").expect("unverified account can resend");
    ...
    assert!(s.verify_email(&target.token).is_some());
    // Once verified, there is nothing to resend.
    assert!(s.issue_resend("alice").is_none());
}

#[test]
fn off_mode_creates_verified_accounts_with_no_token() {
    let store = Arc::new(MemoryStore::new());
    let clock = Arc::new(FixedClock::new(Timestamp(100 * DAY)));
    ...
    assert!(!s.find_user("alice").unwrap().email_enc.contains("alice@example.com"));
}
