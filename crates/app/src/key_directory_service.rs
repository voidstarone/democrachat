...
impl KeyDirectoryService {
    ...
    pub async fn publish_keys(
        ...
        self.keys
            .put_keys(UserKeys::new(user.id, public_key.trim(), wrapped_secret)).await?;
        Ok(())
    }
    ...
    pub async fn my_keys(&self, handle: &str) -> Result<UserKeys, KeyError> {
        let user = self
            .users
            .find_by_handle(handle.trim()).await?
            .ok_or_else(|| KeyError::NoSuchUser(handle.to_string()))?;
        self.keys
            .get_keys(user.id).await?
    ...
    pub async fn public_key_of(&self, handle: &str) -> Result<String, KeyError> {
        let user = self
            .users
            .find_by_handle(handle.trim()).await?
            .ok_or_else(|| KeyError::NoSuchUser(handle.to_string()))?;
