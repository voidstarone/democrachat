...
impl EmojiService {
    ...
    pub fn add_emoji(
        ...
        let user = self
            .users
            .find_by_handle(adder_handle.trim())?
        ...
        if self.emojis.find_emoji(server.id, &name)?.is_some() {
            return Err(EmojiError::NameTaken(name));
        }
        let emoji = Emoji::new(
            self.emojis.next_emoji_id()?,
            server.id,
            name,
            url,
            user.id,
        ...
        self.emojis.insert_emoji(emoji.clone())?;
        Ok(emoji)
    }
    ...
    pub fn vote_emoji(
        ...
        let emoji = self
            .emojis
            .get_emoji(EmojiId(emoji_id))?
            .filter(|e| e.server_id == server.id)
            .ok_or(EmojiError::NoSuchEmoji(emoji_id))?;
        self.emoji_votes
            .upsert_emoji_vote(EmojiVote::new(server.id, emoji.id, user.id, is_up))?;
        Ok(())
    }
    ...
    pub fn ranked_emojis(&self, server_slug: &str, viewer_handle: &str) -> Vec<RankedEmoji> {
        ...
        let mut emojis = self.emojis.list_for_server(server.id).unwrap_or_default();
        ...
        for v in self.emoji_votes.emoji_votes_for_server(server.id).unwrap_or_default() {
        ...
        let mut items: Vec<RankedEmoji> = emojis
            .iter()
            .enumerate()
            .filter(|(i, _)| standings[*i].is_listed())
            .map(|(i, e)| RankedEmoji {
                id: e.id.0,
                name: e.name.clone(),
                url: e.url.clone(),
                ...
                my_vote: viewer.and_then(|v| self.emoji_votes.my_emoji_vote(e.id, v).ok().flatten()),
            })
            .collect();
    ...
    pub fn emoji_name_map(&self, server_slug: &str) -> Vec<(String, String)> {
        match self.servers.find_by_slug(server_slug.trim()).ok().flatten() {
            Some(s) => self
                .emojis
                .list_for_server(s.id).unwrap_or_default()
                .into_iter()
                .map(|e| (e.name, e.url))
                .collect(),
            None => Vec::new(),
