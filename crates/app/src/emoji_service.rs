...
impl Services {
    ...
    pub fn ranked_emojis(&self, server_slug: &str, viewer_handle: &str) -> Vec<RankedEmoji> {
        ...
        let mut emojis = self.emojis.list_for_server(server.id);
        ...
        for v in self.emoji_votes.emoji_votes_for_server(server.id) {
        ...
        let mut items: Vec<RankedEmoji> = emojis
            ...
            .map(|(i, e)| RankedEmoji {
                ...
                my_vote: viewer.and_then(|v| self.emoji_votes.my_emoji_vote(e.id, v)),
            })
            .collect();
