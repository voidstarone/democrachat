...
impl RoleService {
    ...
    pub fn member_handles(&self, server_slug: &str) -> Vec<String> {
        ...
        self.memberships
            ...
            .into_iter()
            .filter_map(|m| self.users.get_user(m.user_id).ok().flatten())
            .map(|u| u.handle)
            .collect()
    }
    ...
    pub fn list_roles(&self, server_slug: &str) -> Vec<Role> {
        match self.servers.find_by_slug(server_slug.trim()).ok().flatten() {
            Some(s) => self.roles.list_for_server(s.id).unwrap_or_default(),
            None => Vec::new(),
        }
    }
    ...
    pub fn role_holders(&self, server_slug: &str, role_name: &str) -> Vec<String> {
        ...
        let Some(role) = self.roles.find_role(server.id, &name).ok().flatten() else {
            return Vec::new();
        };
        self.roles
            .holders(role.id).unwrap_or_default()
            .into_iter()
    ...
    pub fn roles_with_color(&self, server_slug: &str) -> Vec<(Role, Option<RoleColor>)> {
        ...
        let votes = self.role_color_votes.role_color_votes_for_server(server.id).unwrap_or_default();
        self.roles
            .list_for_server(server.id).unwrap_or_default()
            .into_iter()
            .map(|r| {
    ...
    pub fn user_roles(&self, server_slug: &str, handle: &str, viewer_handle: &str) -> Option<UserRoles> {
        ...
        let membership = self.memberships.get(user.id, server.id).ok().flatten()?;
        ...
        let franchised = self.franchised_set(server.id);
        let votes = self.role_color_votes.role_color_votes_for_server(server.id).unwrap_or_default();
        let viewer = self.users.find_by_handle(viewer_handle.trim()).ok().flatten().map(|u| u.id);

        let roles = self
            .roles
            .list_for_server(server.id).unwrap_or_default()
            .into_iter()
            .filter(|r| self.roles.holders(r.id).unwrap_or_default().contains(&user.id))
            .map(|r| {
                let color = winning_color(
                    votes
                        .iter()
                ...
                let my_color = viewer
                    .and_then(|vid| self.role_color_votes.my_role_color_vote(r.id, vid).ok().flatten())
    ...
    fn franchised_set(&self, server: ServerId) -> HashSet<UserId> {
        self.memberships
            .list_for_server(server).unwrap_or_default()
            .into_iter()
            .filter(|m| m.is_franchised())
            .map(|m| m.user_id)
            .collect()
    ...
    fn resolve_one(&self, sid: ServerId, token: String) -> ResolvedMention {
        ...
        if let Some(role) = self.roles.find_role(sid, &token).ok().flatten() {
            let handles = self
                .roles
                .holders(role.id).unwrap_or_default()
                .into_iter()
        ...
        if let Some(user) = self.users.find_by_handle(&token).ok().flatten() {
            if self.memberships.get(user.id, sid).ok().flatten().is_some() {
    ...
    fn members_admitted_by(&self, sid: ServerId, standing: StandingRole) -> Vec<String> {
        self.memberships
            .list_for_server(sid).unwrap_or_default()
            .into_iter()
