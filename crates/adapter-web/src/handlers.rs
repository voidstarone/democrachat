...
pub async fn login(
    ...
    // In hard mode, an account with an unverified email cannot log in. Distinct 403
    // (not the opaque 401 above) so the client can prompt to verify / resend — it
    // only reveals status for a *correct* credential pair, so it is not an existence
    // oracle for wrong guesses.
...
pub async fn register(
    ...
    let reg = st
        .services
        .register_with_password(&req.handle, &req.email, &req.password).await
        .map_err(bad)?;
    st.persist();
...
/// token, marks the account verified, and redirects back into the app with a hash
/// flag the SPA turns into a toast. Unknown/used/expired tokens redirect with an
/// error flag rather than leaking which case occurred.
pub async fn verify(State(st): State<AppState>, Query(q): Query<VerifyQuery>) -> Redirect {
    match st.services.verify_email(&q.token).await {
        Ok(Some(_)) => {
            st.persist();
            Redirect::to("/#verified")
        }
        _ => Redirect::to("/#verify_failed"),
    }
}
